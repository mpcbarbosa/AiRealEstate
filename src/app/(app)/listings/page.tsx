...
import { MapPin, Home, TrendingUp, Filter, LayoutGrid, LayoutList, Star, Phone, RefreshCw } from 'lucide-react'
...
const PIPELINE_STATUSES = ['NONE', 'FAVORITE', 'TO_CONTACT', 'CONTACTED', 'NOT_INTERESTED', 'CLOSED']
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden hover:border-gray-600 transition-colors group">
      <div className="h-40 bg-gray-800 relative overflow-hidden">
        ...
        {userListing?.favorite && (
          <span className="absolute top-2 right-2 text-yellow-400"><Star className="w-4 h-4 fill-current" /></span>
        )}
      </div>
      <div className="p-4">
        ...
        {listing.locationText && (
          <p className="text-xs text-gray-500 flex items-center gap-1 mb-2">
            <MapPin className="w-3 h-3" />{listing.locationText}
          </p>
        )}
        <div className="flex items-center justify-between">
          <span className="text-blue-400 font-semibold">{formatPrice(listing.priceEur)}</span>
          <div className="flex items-center gap-2 text-xs text-gray-500">
            {listing.typology && <span>{listing.typology}</span>}
        ...
        <div className="flex items-center gap-2 mt-3">
          <Link
            ...
            className="flex-1 text-center text-xs bg-gray-800 hover:bg-gray-700 text-gray-300 py-1.5 rounded-lg transition"
          >
            Ver detalhe
          </Link>
          <button
            onClick={() => onPipeline(listing.id, userListing?.status === 'TO_CONTACT' ? 'NONE' : 'TO_CONTACT')}
            className="p-1.5 rounded-lg bg-gray-800 hover:bg-blue-600 text-gray-400 hover:text-white transition"
            title="A contactar"
          >
          ...
          <button
            ...
            className={`p-1.5 rounded-lg bg-gray-800 hover:bg-yellow-600 transition ${userListing?.status === 'FAVORITE' ? 'text-yellow-400' : 'text-gray-400 hover:text-white'}`}
            title="Favorito"
          >
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-white">
            {filters.status ? PIPELINE_LABELS[filters.status] : 'Todos os imóveis'}
          </h1>
          <p className="text-gray-500 text-sm mt-0.5">{pagination.total} imóvel(eis) encontrado(s)</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={fetchListings} className="p-2 rounded-lg bg-gray-800 hover:bg-gray-700 text-gray-400 hover:text-white transition" title="Atualizar">
      ...
      {showFilters && (
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 mb-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div>
              <label className="block text-xs text-gray-400 mb-1">Tipo de negócio</label>
            ...
            <div>
              <label className="block text-xs text-gray-400 mb-1">Tipo de imóvel</label>
          ...
          <div className="mt-3 flex justify-end">
            <button onClick={() => setFilters({ businessType:'',propertyType:'',typology:'',location:'',priceMin:'',priceMax:'',areaMin:'',areaMax:'',keywords:'',status:'',orderBy:'newest',page:1 })}
      ...
      {loading ? (
        ...
        <div className="text-center py-24 text-gray-600">
          ...
          <p className="text-sm mt-1">Ajusta os filtros ou aguarda novos ingest dos agentes Gobii</p>
        </div>
      ) : (
        <>
