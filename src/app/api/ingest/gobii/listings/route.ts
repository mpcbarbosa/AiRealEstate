...
const IngestItemSchema = z.object({
  ...
  locationRegiao: z.string().nullable().optional(),
  locationConcelho: z.string().nullable().optional(),
  locationFreguesia: z.string().nullable().optional(),
  geo: GeoSchema,
...
async function processItem(item: z.infer<typeof IngestItemSchema>, ingestRunId: string) {
  ...
  if (existingSource) {
    ...
    await prisma.listingMaster.update({
      ...
      data: {
        ...
        floor: item.floor ?? master.floor,
        bathrooms: item.bathrooms ?? master.bathrooms,
        hasElevator: item.hasElevator ?? master.hasElevator,
        parkingSpaces: item.parkingSpaces ?? master.parkingSpaces,
        features: (item.features as any) || master.features,
  ...
  if (!existingByHash && item.priceEur && item.areaM2 && item.locationText) {
    ...
    crossSourceMatch = candidates.find(c => isSameListing(c, {
      ...
      locationRegiao: item.locationRegiao || null,
      locationConcelho: item.locationConcelho || null,
      locationFreguesia: item.locationFreguesia || null,
      propertyType,
      businessType,
    })) || null
  }
  ...
  const master = await prisma.listingMaster.create({
    data: {
      ...
      hasElevator: item.hasElevator ?? null,
      parkingSpaces: item.parkingSpaces ?? null,
